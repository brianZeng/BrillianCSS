...
Exp.prototype={
  ...
  get value(){
    ...
    switch (this.type){
      case Exp.TYPE.KEYWORD:case Exp.TYPE.LENGTH:return this.left;
      default:  return this.left.opt(this.optor,this.right);
    }
  },
  reduce:function(){
    ...
    function resolveExp(exp,$vars){
      ...
      switch (ltype){
        case Exp.TYPE.VAR:
          ...
          break;
        case Exp.TYPE.LENGTH:case Exp.TYPE.KEYWORD:
        ret=left;
        break;
        case Exp.TYPE.EXP:case Exp.TYPE.FUNC:
        exp.left=left.resolve($vars);
        return exp.clearVarNames();
        case Exp.TYPE.NONE:throw Error('invalid type');
      }
      switch (rtype){
        case Exp.TYPE.NONE:
          clearRight=true;
  ...
  getVarNames:function(array){
    ...
    array=array||[];
    vars.forEach(function(key){List.arrayAdd(array,key)});
    return array;
  },
  clone:function(){
...
Length.prototype={
  ...
  opt:function(opt,exp){
    ...
    unit=unit||otherUnit;
    switch (opt){
      case '+':return new Length(this.num+num+unit);
      case '-':return new Length(this.num-num+unit);
      case '*':return new Length(this.num*num+unit);
      case '/':return new Length(this.num/num+unit);
      default :throw  'unkonwn optor:'+opt;
    }
  },
...
List.prototype=(function(proto){
  ...
  proto.canResolve=function($vars){
    $vars=$vars||{};
    return this.every(function(obj){ return (obj.canResolve)? obj.canResolve($vars):true;});
  };
  proto.clone=function(){
    return arrayReduce(this,function(list,o){list.push(o.clone? o.clone(true):o);return list;},new List());
  };
  ...
  proto.getVarNames=function(array){
    array=array||[];
    this.forEach(function(o){if(o.hasVars)o.getVarNames(array)});
    return array;
  };
  ...
  proto.reduce=function(callback,initialValue){
    if(arguments.length==0)
      return this.copy(this.map(function(obj){return obj.reduce? obj.reduce():obj}));
    else
...
Scope.prototype={
  ...
  get paramString(){
    var r=[];
    objForEach(this.defValues,function(key,value){r.push(key+':'+value)});
  ...
  add:function(obj){
    switch (obj.type.toLowerCase()){
      case 'rule' :return this.addRule(obj.name,obj.value);
      case 'def': return this.addDefValues(obj.name,obj.value);
      case 'style':return this.addStyle(obj.value);
      case 'include':return this.addInclude(obj.name,obj.value);
      case 'ext':return this.addExt(obj.name);
      case 'mix':return this.addMix(obj.value);
    }
    return this;
  },
  ...
  recordUnresolvedInfo:function($known,unresolved){
    ...
    unresolved.getVarNames().forEach(function(v){if(knownNames.indexOf(v)==-1) List.arrayAdd(r,v);});
  ...
  assign:function(assigns,info,unresolved){
    ...
    do{
      ...
      if(!con)
        con=Object.getOwnPropertyNames(unres).some(function(key){return unres[key].canResolve(result); });
    }while(con);
    if(info){
      var known=Object.getOwnPropertyNames(result);
      objForEach(unres,function(key,value){info[key]=this.recordUnresolvedInfo(known,value);},this);
    }
  ...
  resolve:(function(){
    ...
    function resolveInclude(mixin,$param,selfRules,arg){
      ...
      objForEach(selfRule,function(key,value){selfRules[key]=value});
...
Sheet.prototype=(function(proto){
  ...
  sheetProto.resolve=function($vars,info){
    ...
    styles.forEach(function(s){ctx.setExtends(s);});
