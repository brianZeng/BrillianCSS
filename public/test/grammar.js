  ...
  describe('a.normal body rules', function () {
    it('1.can parse complex body', function () {
      src = 'input[type=range]{' +
        'background: -webkit-linear-gradient(left,#1d2e38 0%,#2b4254 50%,#2b4254 100%) !important;' +
        '}';
      scope = getFirstScope(src);
      expect(scope.staticRules).toEqual({background: '-webkit-linear-gradient(left,#1d2e38 0%,#2b4254 50%,#2b4254 100%) !important'});
      src = 'input[type=range]{' +
        'background: 0px 2px 10px 0px #000000 inset,1px 0px 2px rgba(0, 0, 0, 0.4) inset ; }';
      scope = getFirstScope(src);
      expect(scope.staticRules).toEqual({background: '0px 2px 10px 0px #000000 inset , 1px 0px 2px rgba(0,0,0,0.4) inset'});
